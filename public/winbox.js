...
(function () {
  ...
  function S(a, b) {
    ...
    this.g.className =
      'winbox' + (O ? ' ' + ('string' === typeof O ? O : O.join(' ')) : '') + (g ? ' modal' : '');
    this.x = u;
    this.y = c;
    this.width = n;
    this.height = q;
  ...
  function T() {
    ...
    N =
      I &&
      I.replace('request', 'exit').replace('mozRequest', 'mozCancel').replace('Request', 'Exit');
  ...
  e.minimize = function (a) {
    ...
    !a && this.min
      ...
      : !1 === a ||
        this.min ||
        (w.push(this), V(), (this.g.title = this.title), this.addClass('min'), (this.min = !0));
  ...
  e.move = function (a, b, c) {
    a || 0 === a
      ...
      : ((a = this.x),
        (b = this.y),
        this.v &&
          (0 === a ? this.resize(P / 2, Q) : a === P - this.width && this.resize(P / 2, Q)));
